...
> How do applications and artificial intelligence systems gain trust? - this seems to be one of the most important and difficult questions that software engineering must answer today.
...
Well, trust will certainly be built based on the answers applications provide. And computer programs produce different types of responses and results depending on the type of application they belong to. 
So, let's discuss the types of results generated by apps by type:
...
That breaks into three key ideas of trust builders:
...
But we probably won't make a big mistake if we assume that for **Reliability** we expect identical or very similar behavior for each application type.
...
Works under load, over time, and with failures
...
- Predictability → observable behavior, event ordering (when required), and idempotent processing, even in edge cases (e.g., message loss, duplicates)
...
**Trust keys**:
- Correctness → data accuracy and integrity, and correct commit/rollback behavior
...
**Trust keys**:
- Correctness → safety and correct physical behavior
- Reliability → fault tolerance and long-term operation without failure
- Predictability → fail-safe behavior and clear error states, even in edge cases (e.g., power loss, sensor failure)
...
- Reliability → CIA triad: confidentiality, integrity, and availability
...
- Reliability → scalability and consistent performance over time
- Predictability → isolation between services and clear error handling, even in edge cases (e.g., unexpected inputs, model drift)
...
> - The system guarantees defined behavior under failure.
...
> It assumes: “Failures are normal and handled”


We don’t “trust a program” blindly. Our trust it based on:
...
> **Trust is not a property of correctness alone—it is a property of controlled failure, observable behavior, and guaranteed recovery semantics.**
